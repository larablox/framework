...
- `reports/parity/` - `npm run parity` output (per-member mirror fidelity, whole tree); gitignored
...
- To port an upstream file or directory, use the `port-upstream` skill
  (`.claude/skills/port-upstream/SKILL.md`). It ends with a mandatory
  feedback entry in `FEEDBACK.md` next to it - read that file before
  starting a port.

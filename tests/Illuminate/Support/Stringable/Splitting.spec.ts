...
export = (): void => {
    describe('Stringable splitting', () => {
        it('explode() splits the string into a collection', () => {
            ...
            expect(arraysEqual(Str.of('Foo Bar Baz').explode(' ').all(), [
                'Foo',
                'Bar',
                'Baz',
            ])).to.equal(true);
            ...
            // with limit
            expect(arraysEqual(Str.of('Foo Bar Baz').explode(' ', 2).all(), [
                'Foo',
                'Bar Baz',
            ])).to.equal(true);
            expect(arraysEqual(Str.of('Foo Bar Baz').explode(' ', -1).all(), [
                'Foo',
                'Bar',
            ])).to.equal(true);
        });

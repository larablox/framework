...
export = (): void => {
    describe("Collection sorting", () => {
        it("sort() orders values naturally", () => {
            ...
            expectDeepEqual(new Collection([5, 3, 1, 2, 4]).sort().values().all(), [1, 2, 3, 4, 5]);

            expectDeepEqual(new Collection([-1, -3, -2, 0, 2, 1]).sort().values().all(), [-3, -2, -1, 0, 1, 2]);
        });

        it("sortDesc() orders values in reverse", () => {
            // PHP: SupportCollectionTest::testSortDesc
            expectDeepEqual(new Collection([5, 3, 1, 2, 4]).sortDesc().values().all(), [5, 4, 3, 2, 1]);
        });

        it("sort() accepts a comparator callback", () => {
            // PHP: SupportCollectionTest::testSortWithCallback
            const data = new Collection([5, 3, 1, 2, 4]).sort((first, second) => first - second);
        ...
        it("sortBy() / sortByDesc() order by the result of a callback or a key", () => {
            ...
            const data = new Collection(["taylor", "dayle"]).sortBy((name) => name);
            expectDeepEqual(data.values().all(), ["dayle", "taylor"]);

            const desc = new Collection(["dayle", "taylor"]).sortByDesc((name) => name);
            expectDeepEqual(desc.values().all(), ["taylor", "dayle"]);

            const byKey = new Collection([{ name: "taylor" }, { name: "dayle" }]).sortBy("name");
            expectDeepEqual(byKey.values().all(), [{ name: "dayle" }, { name: "taylor" }]);
        });

...
export = (): void => {
    describe("Stringable replacing", () => {
        it("replace() replaces every occurrence", () => {
            ...
            expect(Str.of("?/?/?").replace("?", "foo").toString()).to.equal(
                "foo/foo/foo",
            );
            expect(Str.of("?/?").replace("?", "bar").toString()).to.equal(
                "bar/bar",
            );
        ...
        it("replaceFirst()/replaceStart() replace the leading occurrence", () => {
            ...
            expect(
                Str.of("foobar foobar").replaceFirst("bar", "qux").toString(),
            ).to.equal("fooqux foobar");
            expect(
                Str.of("foobar foobar").replaceFirst("bar", "").toString(),
            ).to.equal("foo foobar");
            expect(
                Str.of("foobar foobar").replaceFirst("xxx", "yyy").toString(),
            ).to.equal("foobar foobar");
            expect(
                Str.of("foobar foobar").replaceFirst("", "yyy").toString(),
            ).to.equal("foobar foobar");
            expect(
                Str.of("Jönköping Malmö").replaceFirst("ö", "xxx").toString(),
            ).to.equal("Jxxxnköping Malmö");

            expect(
                Str.of("foobar foobar").replaceStart("bar", "qux").toString(),
            ).to.equal("foobar foobar");
            expect(
                Str.of("foobar foobar").replaceStart("foo", "qux").toString(),
            ).to.equal("quxbar foobar");
            expect(
                Str.of("foobar foobar").replaceStart("foo", "").toString(),
            ).to.equal("bar foobar");
            expect(Str.of("0").replaceStart("0", "1").toString()).to.equal("1");
            expect(
                Str.of("Jönköping Malmö").replaceStart("Jö", "xxx").toString(),
            ).to.equal("xxxnköping Malmö");
        });

        it("replaceLast()/replaceEnd() replace the trailing occurrence", () => {
            // PHP: SupportStringableTest::testReplaceLast / testReplaceEnd
            expect(
                Str.of("foobar foobar").replaceLast("bar", "qux").toString(),
            ).to.equal("foobar fooqux");
            expect(
                Str.of("foobar foobar").replaceLast("bar", "").toString(),
            ).to.equal("foobar foo");
            expect(
                Str.of("foobar foobar").replaceLast("xxx", "yyy").toString(),
            ).to.equal("foobar foobar");
            expect(
                Str.of("Malmö Jönköping").replaceLast("ö", "xxx").toString(),
            ).to.equal("Malmö Jönkxxxping");

            expect(
                Str.of("foobar foobar").replaceEnd("bar", "qux").toString(),
            ).to.equal("foobar fooqux");
            expect(
                Str.of("foobar foobar").replaceEnd("bar", "").toString(),
            ).to.equal("foobar foo");
            expect(
                Str.of("fooxxx foobar").replaceEnd("xxx", "yyy").toString(),
            ).to.equal("fooxxx foobar");
            expect(
                Str.of("Malmö Jönköping").replaceEnd("ö", "xxx").toString(),
            ).to.equal("Malmö Jönköping");
            expect(
                Str.of("Malmö Jönköping").replaceEnd("öping", "yyy").toString(),
            ).to.equal("Malmö Jönkyyy");
        });
        ...
        it("remove() removes any occurrence of the given string(s)", () => {
            ...
            expect(Str.of("Foobar").remove(["o", "a"]).toString()).to.equal(
                "Fbr",
            );
            expect(Str.of("Foobar").remove(["f", "b"]).toString()).to.equal(
                "Fooar",
            );
            expect(Str.of("Foo|bar").remove(["f", "|"]).toString()).to.equal(
                "Foobar",
            );
        });
        ...
        it("deduplicate() collapses runs of a repeated character", () => {
            ...
            expect(
                Str.of(" laravel   php  framework ").deduplicate().toString(),
            ).to.equal(" laravel php framework ");
            expect(Str.of("whaaat").deduplicate("a").toString()).to.equal(
                "what",
            );
            expect(
                Str.of("/some//odd//path/").deduplicate("/").toString(),
            ).to.equal("/some/odd/path/");
            expect(Str.of("ムだだム").deduplicate("だ").toString()).to.equal(
                "ムだム",
            );
            expect(
        ...
        it("replaceMatches() replaces every match, with a string or a callback", () => {
            ...
            const result = Str.of("Hello world!").replaceMatches(
                "world",
                (match) => Str.upper(match),
            );

            expect(result.toString()).to.equal("Hello WORLD!");

            const limited = Str.of("apple orange apple").replaceMatches(
                "apple",
                "fruit",
                1,
            );

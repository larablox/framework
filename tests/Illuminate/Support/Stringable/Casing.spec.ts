...
export = (): void => {
    describe("Stringable casing", () => {
        it("kebab() converts to kebab case", () => {
            ...
            expect(Str.of("LaravelPhpFramework").kebab().toString()).to.equal(
        ...
        it("lower()/upper() fold ASCII case", () => {
            ...
            expect(Str.of("FOO BAR BAZ").lower().toString()).to.equal(
                "foo bar baz",
            );
            expect(Str.of("fOo Bar bAz").lower().toString()).to.equal(
                "foo bar baz",
            ...
            expect(Str.of("foo bar baz").upper().toString()).to.equal(
                "FOO BAR BAZ",
            );
            expect(Str.of("foO bAr BaZ").upper().toString()).to.equal(
                "FOO BAR BAZ",
        ...
        it("title() titles every word", () => {
            ...
            expect(Str.of("jefferson costella").title().toString()).to.equal(
                "Jefferson Costella",
            );
            expect(Str.of("jefFErson coSTella").title().toString()).to.equal(
                "Jefferson Costella",
        ...
        it("ucwords() upper-cases the first character of every word", () => {
            ...
            expect(Str.of("laravel framework").ucwords().toString()).to.equal(
                "Laravel Framework",
            );
            expect(
                Str.of("laravel-framework").ucwords("-").toString(),
            ).to.equal("Laravel-Framework");
            ...
            expect(Str.of("мама мыла раму").ucwords().toString()).to.equal(
                "Мама Мыла Раму",
            );
        ...
        it("ucsplit() splits a string on its upper case characters", () => {
            ...
            expect(
                arraysEqual(Str.of("TaylorOtwell").ucsplit().all(), [
                    "Taylor",
                    "Otwell",
                ]),
            ).to.equal(true);
            expect(
                arraysEqual(Str.of("HelloFromLaravel").ucsplit().all(), [
                    "Hello",
                    "From",
                    "Laravel",
                ]),
            ...
            expect(
                arraysEqual(Str.of("He_llo_World").ucsplit().all(), [
                    "He_llo_",
                    "World",
                ]),
            ).to.equal(true);
        ...
        it("snake() converts to snake case", () => {
            ...
            expect(Str.of("LaravelPHPFramework").snake().toString()).to.equal(
            ...
            expect(
                Str.of("LaravelPhpFramework").snake(" ").toString(),
            ).to.equal("laravel php framework");
            expect(Str.of("Laravel Php Framework").snake().toString()).to.equal(
                "laravel_php_framework",
            );
            // ensure cache keys don't overlap
            expect(
                Str.of("LaravelPhpFramework").snake("__").toString(),
            ).to.equal("laravel__php__framework");
            expect(
                Str.of("LaravelPhpFramework_").snake("_").toString(),
            ).to.equal("laravel_php_framework_");
            ...
            expect(Str.of("ŻółtaŁódka").snake().toString()).to.equal(
                "żółtałódka",
            );
        });
        ...
        it("studly()/pascal() convert to studly/pascal case", () => {
            ...
            expect(
                Str.of("laravel_p_h_p_framework").studly().toString(),
            ).to.equal("LaravelPHPFramework");
            expect(
                Str.of("laravel-phP-framework").studly().toString(),
            ).to.equal("LaravelPhPFramework");
            expect(Str.of("fooBar").studly().toString()).to.equal("FooBar");
            expect(Str.of("foo-barBaz").studly().toString()).to.equal(
                "FooBarBaz",
            );

            expect(
                Str.of("laravel_p_h_p_framework").pascal().toString(),
            ).to.equal("LaravelPHPFramework");
            expect(
                Str.of("laravel-phP-framework").pascal().toString(),
            ).to.equal("LaravelPhPFramework");
            expect(Str.of("fooBar").pascal().toString()).to.equal("FooBar");
            expect(Str.of("foo-barBaz").pascal().toString()).to.equal(
                "FooBarBaz",
            );
        });
        ...
        it("camel() converts to camel case", () => {
            ...
            expect(
                Str.of("Laravel_p_h_p_framework").camel().toString(),
            ).to.equal("laravelPHPFramework");
            expect(Str.of("Laravel-phP-framework").camel().toString()).to.equal(
                "laravelPhPFramework",
            );
            expect(Str.of("FooBar").camel().toString()).to.equal("fooBar");
            expect(Str.of("Foo-barBaz").camel().toString()).to.equal(
                "fooBarBaz",
            );
        });
        ...
        it("initials() takes the first letter of every word", () => {
            ...
            expect(Str.of("Taylor Otwell").initials().toString()).to.equal(
                "TO",
            );
            expect(Str.of("taylor otwell").initials().toString()).to.equal(
                "to",
            );
            expect(Str.of("taylor otwell").initials(true).toString()).to.equal(
                "TO",
            );
            expect(Str.of("james bond").initials(true).toString()).to.equal(
                "JB",
            );
        });
    });

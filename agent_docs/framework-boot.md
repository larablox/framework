...
Осознанно оставлено общим: `Router::$currentRoute` и `Router::$currentRequest`.
Они верны только для того запроса, который записал их последним. Сделать их
per-request нечем — нужен per-coroutine store; правильный аксессор под
чередованием — `Request::route()`, он идёт через `setRouteResolver()` и всегда
указывает на копию своего запроса. `Router::current()`, `currentRouteName()`,
`currentRouteAction()` и фасадные обёртки над ними этой гарантии не дают.

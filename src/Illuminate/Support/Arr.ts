...
/** PHP: `fn ($value, $key)`. */
export type ArrCallback<TValue, TReturn> = (
    value: TValue,
    key: string | number,
) => TReturn;
...
export class Arr {
    ...
    public static wrap<T extends defined>(
    ...
    public static dot(
        ...
        const flatten = (
            ...
            for (const [key, value] of pairs(data)) {
                ...
                const nested =
                    Arr.accessible(value) && currentDepth < depth
                        ? (value as ArrayAccessible)
                        : undefined;
    ...
    public static sole<T extends defined>(
        list: Array<T>,
        callback?: ArrCallback<T, boolean>,
    ): T {
        const matched =
            callback === undefined ? list : Arr.where(list, callback);
    ...
    public static take<T extends defined>(
        list: Array<T>,
        limit: number,
    ): Array<T> {
        if (limit < 0) {
    ...
    public static every<T extends defined>(
        list: Array<T>,
        callback: ArrCallback<T, boolean>,
    ): boolean {
        for (let index = 0; index < list.size(); index++) {
    ...
    public static some<T extends defined>(
        list: Array<T>,
        callback: ArrCallback<T, boolean>,
    ): boolean {
        for (let index = 0; index < list.size(); index++) {
    ...
    public static where<T extends defined>(
        list: Array<T>,
        callback: ArrCallback<T, boolean>,
    ): Array<T> {
    ...
    public static reject<T extends defined>(
        list: Array<T>,
        callback: ArrCallback<T, boolean>,
    ): Array<T> {
    ...
    public static whereNotNull<T extends defined>(
    ...
    public static mapSpread<TResult extends defined>(
        list: Array<Array<defined>>,
        callback: Callback,
    ): Array<TResult> {
    ...
    public static keyBy<T extends defined>(
        list: Array<T>,
        keyBy: string | ArrCallback<T, string>,
    ): ArrayAccessible {
        const result: ArrayAccessible = {};
     ...
     */
    public static pluck<T extends defined>(
        list: Array<T>,
        value: string,
    ): Array<defined> {
    ...
    public static select<T extends defined>(
        list: Array<T>,
        keys: string | Array<string>,
    ): Array<ArrayAccessible> {
        const wanted = Util.arrayWrap(keys);
    ...
    public static onlyValues<T extends defined>(
        list: Array<T>,
        values: T | Array<T>,
    ): Array<T> {
    ...
    public static exceptValues<T extends defined>(
        list: Array<T>,
        values: T | Array<T>,
    ): Array<T> {
    ...
    public static prepend<T extends defined>(
        list: Array<T>,
        value: T,
    ): Array<T> {
    ...
    public static flatten(
        ...
        for (const item of list) {
            ...
            const values =
                depth === 1
                    ? (item as Array<defined>)
                    : Arr.flatten(item as Array<defined>, depth - 1);
    ...
    public static random<T extends defined>(
        list: Array<T>,
        count?: number,
    ): T | Array<T> | undefined {
    ...
    public static sortDesc<T extends defined>(
        list: Array<T>,
        callback?: string | ArrCallback<T, defined>,
    ): Array<T> {
    ...
    public static sortRecursive(
        ...
        return Arr.map(sorted, (value) =>
            Util.isArray(value)
                ? Arr.sortRecursive(value as Array<defined>, descending)
                : value,
        );
    }
    ...
    public static from(items: unknown): Array<defined> {
        ...
        if (
            typeIs(items, "table") &&
            typeIs((items as { all?: unknown }).all, "function")
        ) {
            return ((items as { all: Callback }).all as Callback)(
                items,
            ) as Array<defined>;
        }
    ...
    private static slice<T extends defined>(
        list: Array<T>,
        start: number,
        length: number,
    ): Array<T> {
        ...
        for (
            let index = start;
            index < math.min(start + length, list.size());
            index++
        ) {
            result.push(list[index]);
        }

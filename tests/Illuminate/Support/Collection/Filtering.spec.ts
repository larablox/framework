...
export = (): void => {
    describe("Collection filtering", () => {
        it("filter() keeps items passing a truth test, or truthy items with no callback", () => {
            ...
            expectDeepEqual(c.filter((item) => item.id === 2).all(), [
                { id: 2, name: "World" },
            ]);

            const truthy = new Collection(["", "Hello", "", "World"]);
            expectDeepEqual(truthy.filter().values().toArray(), [
                "Hello",
                "World",
            ]);
        });
        ...
        it("reject() removes items passing a truth test, or truthy items with no callback", () => {
            ...
            const data1 = new Collection<number, boolean | number>([
                false,
                true,
                0,
            ]);
            expectDeepEqual(data1.reject((value) => Util.truthy(value)).all(), [
                false,
                0,
            ]);
            ...
            expect(
                data2.reject((value) => Util.truthy(value)).isEmpty(),
            ).to.equal(true);
        });
        ...
        it("where() filters items by a key/value pair, with an operator or none", () => {
            ...
            expectDeepEqual(c.where("v", "<>", 3).values().all(), [
                { v: 1 },
                { v: 2 },
                { v: 4 },
            ]);
            ...
            expectDeepEqual(g.where("v", 2).where("g", 3).values().all(), [
                { v: 2, g: 3 },
            ]);
        ...
        it("whereIn() / whereNotIn() filter by membership in a value list", () => {
            ...
            expectDeepEqual(c.whereIn("v", [1, 3]).values().all(), [
                { v: 1 },
                { v: 3 },
            ]);
            expectDeepEqual(
            ...
            expectDeepEqual(c.whereNotIn("v", [1, 3]).values().all(), [
                { v: 2 },
                { v: 4 },
            ]);
        });
        ...
        it("whereNull() / whereNotNull() filter by a key being undefined or not", () => {
            ...
            expectDeepEqual(data.whereNull("name").values().all(), [
                { name: undefined },
            ]);
            expectDeepEqual(data.whereNotNull("name").values().all(), [
                { name: "Taylor" },
                { name: "Bert" },
            ]);
        });
